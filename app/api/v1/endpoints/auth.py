...
from app.schemas.auth_schema import SignupResponse, Token, CreateUser, LogoutResponse, ForgotPassword
...
async def forgot_password(payload: ForgotPassword,
						  ...
						  auth_service: AuthService = Depends(get_auth_service),
						  email_service: EmailService = Depends(get_email_service)):
	user = await auth_service.forgot_password(str(payload.email))
	tasks.add_task(email_service.send_forgot_password_email, user)
	return {
		"success": True,
